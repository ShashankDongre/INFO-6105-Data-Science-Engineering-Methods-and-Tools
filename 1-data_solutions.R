...
college = read.csv("C:\\Users\\shash\\OneDrive\\Desktop\\Northeastern\\SEM 2\\Data Science Engineering Methods and Tools\\Class Material and PPTs\\W1L2\\RLab6105\\data\\College.csv")
## overview of dataset
head(college)
dim(college)
summary(college)
...
filter(df, Private=="Yes" & Grad.Rate >= 80)
...
df_new = arrange(df, desc(Grad.Rate))
df_new2= select(df_new, X, Grad.Rate)
slice_head(df_new2, n=9)
...
## ✖ Column `Agency.Name` doesn't exist.
...
## max and min tuition ("Outstate") grouped by private/public
## school, in dataset 'df'

tutionmaxmin = group_by(df, Private)
tutionmaxmin = summarise(tutionmaxmin, max = max(Outstate),min = min(Outstate))
tutionmaxmin



## max and min tuition ("Outstate") grouped by private/public
## school, in dataset 'College'

tutionmaxmin = group_by(college, Private)
tutionmaxmin = summarise(tutionmaxmin, max = max(Outstate),min = min(Outstate))
tutionmaxmin
